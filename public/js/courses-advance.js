...
function addCourse() {
  ...
  let courseSemester = document.getElementById('courseSemester').value,;
...
function getAllCourses() {
  db.collection("courses").get().then((querySnapshot) => {
    querySnapshot.forEach((doc) => {
      ...
      if (doc.data().courseAuthor === currentUser.uid) {
        // Create `div` Element
        let div = document.createElement('div');
        // Check if courseSemester Exist
        let courseSemester = "";
        if (doc.data().courseSemester !== undefined) {
          // Set courseSemester to doc.data().courseSemester
          courseSemester = " - " + doc.data().courseSemester;
        }
        ...
        div.innerHTML =
          '<div class="card">\
            ...
            <div class="card-body">\
              <h5 class="card-title">' + doc.data().courseNumber + courseSemester + '</h5>\
